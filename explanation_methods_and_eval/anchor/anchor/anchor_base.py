...
#         print 'lm', lm, 'qm', qm, kl_bernoulli(p, qm)
    ...
    def lucb(self, sample_fns, initial_stats, epsilon, delta, batch_size, top_n,
             verbose=False, verbose_every=1, max_num_samples=1000):
        ...
        self.num_forward += sum(n_samples)
        t = 1
        if len(means) == 0:
            print(means)
        ...
        B = ub[ut] - lb[lt]
        verbose_count = 0
        # print(self.num_forward)
        # print(max_num_samples)
        # print(B)
        # print(epsilon)
        # import pdb; pdb.set_trace()
        while B > epsilon and self.num_forward <= max_num_samples:
            verbose_count += 1
            if verbose and verbose_count % verbose_every == 0:
                ...
                print('B = %.2f | n_total = %d' % (B, self.num_forward))
    ...
    def anchor_beam(self, sample_fn, delta=0.05, epsilon=0.1, batch_size=10,
        ...
        while current_size <= max_anchor_size:
            ...
            chosen_tuples = self.lucb(
                sample_fns, initial_stats, epsilon, delta, batch_size,
                min(beam_size, len(tuples)), 
                verbose=verbose, verbose_every=verbose_every, max_num_samples=max_num_samples)
            ...
            for i, t in zip(chosen_tuples, best_of_size[current_size]):
                ...
                while ((mean >= desired_confidence and
                    ...
                    ub = AnchorBaseBeam.dup_bernoulli(
                        mean, beta / state['t_nsamples'][t])
                    self.num_forward += batch_size
                    if self.num_forward >= max_num_samples:
                        break
                if verbose:
                    print('%s mean = %.2f lb =% .2f ub = %.2f coverage: %.2f n: %d | n_total: %d' % (t, mean, lb, ub, coverage, state['t_nsamples'][t], self.num_forward))
                # print(sum(state['t_nsamples']))
                if self.num_forward >= max_num_samples - batch_size:
                    break
