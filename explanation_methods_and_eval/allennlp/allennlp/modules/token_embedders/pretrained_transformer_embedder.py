...
class PretrainedTransformerEmbedder(TokenEmbedder):
    ...
    def forward(
        ...
        mask: Union[torch.BoolTensor, torch.FloatTensor],
        ...
        mask: `torch.BoolTensor` or `torch.FloatTensor`
        ...
        if transformer_mask.dtype != torch.float:
            transformer_mask = transformer_mask.float()
        ...
        if self._scalar_mix is not None:
            ...
            sequence_embeddings = transformer_output[0]
            pooled_embeddings = transformer_output[1]

        if fold_long_sequences:
            sequence_embeddings = self._unfold_long_sequences(
                sequence_embeddings, segment_concat_mask, batch_size, num_segment_concat_wordpieces
            )
    ...
    def _unfold_long_sequences(
        ...
        if not (lengths_to_mask(seq_lengths, mask.size(1), device) == mask).all():
            ...
            # )
            # If the assumption that all 1s precede all 0s is not satisfied, find the position
            # of the last non zero element in the sequence and use that as sequence length.
            # This can be time and memory expensive.
            batch_num, seq_pos = mask.nonzero(as_tuple=True)
            frequencies = torch.bincount(batch_num)
            last_non_zero_pos = torch.cumsum(frequencies, dim=0) - 1
            seq_lengths = seq_pos[last_non_zero_pos]
