...
class GradientDescentTrainer(Trainer):
    ...
    def random_mask(self, size, prop, pad_to=None, ignore_positions=None):
        # prop is proportion of zeros
        array = torch.ones(size)
        ...
        if pad_to is not None:
            padding = torch.ones(pad_to-size)
    ...
    def batch_outputs(self, batch: TensorDict, for_training: bool, masking_augmentation: bool = False, PLS_masks: bool = False) -> Dict[str, torch.Tensor]:
        ...
        if masking_augmentation:
            ...
            batch['always_keep_mask'] = torch.cat((always_keep_mask, always_keep_mask), dim=0)
            batch['label'] = torch.cat((labels, labels), dim=0)
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        done_early = False
        for batch_no, batch_group in enumerate(batch_group_generator_tqdm):
            if self._distributed:
            ...
            for _, batch in enumerate(batch_group):
                ...
                # print("value: ", batch_outputs.get("binary_mask"))
                # print("gradient: ", batch_outputs.get("binary_mask").grad)
                # assert 1 == 2
    ...
    def _validation_loss(self, epoch: int) -> Tuple[float, float, int]:
        ...
        for batch in val_generator_tqdm:
            ...
            with amp.autocast(self._use_amp):
                batch_outputs = self.batch_outputs(batch, for_training=False, masking_augmentation=False)
                loss = batch_outputs.get("loss")
                reg_loss = batch_outputs.get("reg_loss")
                if loss is not None:
