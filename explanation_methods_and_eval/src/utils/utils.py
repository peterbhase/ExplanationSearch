...
def load_untokenized_documents(data_dir: str, docids: Set[str]) -> Dict[str, str]:
    """ Loads documents as untokenized, continuous string specified by docids.

    Returns a dictionary with docid as key and document string as value.
    """
    docs_dir = os.path.join(data_dir, 'docs')
    res = dict()
    if docids is None:
